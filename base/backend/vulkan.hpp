// This is a one-stop shop Vulkan implementation. All Vulkan code should reside
// in the base/backend folder. All information on the API that was used to
// produce this implementation has been garnered from the following primary
// resources:
//
...
make_shader_stage_settings(VkShaderModule module, VkShaderStageFlagBits type);
...
struct queue_family_indices {
  ...
  bool ok() const {
    bool r = graphics_family.has_value() && present_family.has_value();
    ASSERT(r);
    return r;
  }
};
...
struct series_gen {
  ...
  bool make(uniform_block_data<blockStructType> &block, uint32_t binding_index,
            uint32_t array_elem_index = 0) {
    bool ret{false};
    if (c_assert(properties.ok()) && c_assert(pool != nullptr) &&
...
static inline VkPushConstantRange range(uint32_t offset = 0) {
  return {flags, offset, sizeof(T)};
}

template <class T, VkShaderStageFlags flags>
static inline void upload(T *ptr, VkCommandBuffer cmd_buffer,
                          VkPipelineLayout layout, uint32_t offset = 0) {
  vkCmdPushConstants(cmd_buffer, layout, flags, offset, sizeof(T), ptr);
}

static inline VkPushConstantRange basic_pbr_range() {
  return range<basic_pbr, VK_SHADER_STAGE_FRAGMENT_BIT>();
}

static inline VkPushConstantRange model_range() {
  return range<model, VK_SHADER_STAGE_VERTEX_BIT>(k_model_offset);
}

static inline void basic_pbr_upload(basic_pbr &pc, VkCommandBuffer cmd_buffer,
                                    VkPipelineLayout layout) {
  upload<basic_pbr, VK_SHADER_STAGE_FRAGMENT_BIT>(&pc, cmd_buffer, layout);
}

static inline void model_upload(model &pc, VkCommandBuffer cmd_buffer,
                                VkPipelineLayout layout) {
  upload<model, VK_SHADER_STAGE_VERTEX_BIT>(&pc, cmd_buffer, layout,
                                            k_model_offset);
}
...
struct buffer_data {
  ...
  bool ok() const { return c_assert(H_OK(handle)) && c_assert(H_OK(memory)); }
...
class renderer {
  ...
  struct model_data {
    ...
    std::unordered_map<std::string, uint32_t>
        indices{}; // into the above buffers

    size_t length() const { return transforms.size(); }
  ...
  darray<image_pool::index_type> m_test_image_indices = {image_pool::k_unset,
                                                         image_pool::k_unset};

  darray<texture_pool::index_type> m_test_texture_indices = {
      texture_pool::k_unset, texture_pool::k_unset};
  ...
  darray<pipeline_layout_pool::index_type> m_pipeline_layout_indices = {
      pipeline_layout_pool::k_unset, pipeline_layout_pool::k_unset,
      pipeline_layout_pool::k_unset};

  darray<pipeline_pool::index_type> m_pipeline_indices = {
      pipeline_pool::k_unset, pipeline_pool::k_unset, pipeline_pool::k_unset};
  ...
  static inline darray<vk_layer_info> s_layers = {
      {"VK_LAYER_LUNARG_standard_validation",
       st_config::c_renderer::k_enable_validation_layers},
  ...
  VkPipelineLayout pipeline_layout(int index) const {
    return m_pipeline_layout_pool.pipeline_layout(
        m_pipeline_layout_indices.at(index));
  }

  VkPipeline pipeline(int index) const {
    return m_pipeline_pool.pipeline(m_pipeline_indices.at(index));
  }

  VkDescriptorSet descriptor_set(int index) const {
    return m_descriptor_set_pool.descriptor_set(
        m_test_descriptor_set_indices.at(index));
  }

  darray<VkDescriptorSet> descriptor_sets(
      const darray<descriptor_set_pool::index_type> &indices) const {
    return c_fmap<descriptor_set_pool::index_type, VkDescriptorSet>(
        indices, [this](const descriptor_set_pool::index_type &i) {
          return descriptor_set(i);
        });
  }

  VkDescriptorSetLayout descriptor_set_layout(int index) const {
    return m_descriptor_set_pool.descriptor_set_layout(
        m_test_descriptor_set_indices.at(index));
  }

  void print_physical_device_memory_types() {
    darray<VkMemoryType> mem_types =
        get_physical_device_memory_types(m_vk_curr_pdevice);
  ...
  darray<VkMemoryType>
  get_physical_device_memory_types(VkPhysicalDevice device) {
  ...
  device_resource_properties make_device_resource_properties() const {
    ...
    return {std::move(indices_copy),    m_vk_curr_pdevice,    m_vk_curr_ldevice,
            query_queue_sharing_mode(), m_vk_descriptor_pool, m_vk_command_pool,
            m_vk_graphics_queue};
  }

  queue_family_indices query_queue_families(VkPhysicalDevice device,
                                            VkSurfaceKHR surface) const {
    queue_family_indices indices{};

    if (ok()) {
      uint32_t queue_fam_count = 0;
      vkGetPhysicalDeviceQueueFamilyProperties(device, &queue_fam_count,
                                               nullptr);
  ...
  VkBufferCreateInfo make_vertex_buffer_create_info(VkDeviceSize size) const {
    ...
    buffer_info.pQueueFamilyIndices =
        query_graphics_buffer_indices(&num_indices);
  ...
  image_pool::index_type make_framebuffer_attachment(VkFormat format,
    ...
    ret = m_image_pool.make_image(make_device_resource_properties(), gen_image);
    ...
    /*
      ...
      // VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT flag is required for input
      attachments; imageCI.usage = usage | VK_IMAGE_USAGE_INPUT_ATTACHMENT_BIT;
      ...
      &attachment->memory)); VK_CHECK_RESULT(vkBindImageMemory(device,
      attachment->image, attachment->memory, 0));

      VkImageViewCreateInfo imageViewCI =
      vks::initializers::imageViewCreateInfo(); imageViewCI.viewType =
      VK_IMAGE_VIEW_TYPE_2D; imageViewCI.format = format;
      ...
      VK_CHECK_RESULT(vkCreateImageView(device, &imageViewCI, nullptr,
      &attachment->view));
    */
  }

  std::optional<VkMemoryRequirements>
  query_vertex_buffer_memory_requirements(VkDeviceSize size) const {
    ...
    if (ok_graphics_pipeline()) {
      ...
      VK_FN(vkCreateBuffer(m_vk_curr_ldevice, &buffer_info, nullptr,
                           &dummy_buffer));

      if (ok() && dummy_buffer != VK_NULL_HANDLE) {
        VkMemoryRequirements r = {};
        vkGetBufferMemoryRequirements(m_vk_curr_ldevice, dummy_buffer, &r);
    ...
    if (m_vk_khr_surface != VK_NULL_HANDLE) {
      if (ok()) {
        VK_FN(vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
            device, m_vk_khr_surface, &details.capabilities));
      }
      if (ok()) {
        uint32_t format_count = 0;
        VK_FN(vkGetPhysicalDeviceSurfaceFormatsKHR(device, m_vk_khr_surface,
                                                   &format_count, nullptr));
        if (ok()) {
          ASSERT(format_count != 0);
          details.formats.resize(format_count);
          VK_FN(vkGetPhysicalDeviceSurfaceFormatsKHR(
              device, m_vk_khr_surface, &format_count, details.formats.data()));
        }
      }
      if (ok()) {
        uint32_t present_mode_count = 0;
        VK_FN(vkGetPhysicalDeviceSurfacePresentModesKHR(
            device, m_vk_khr_surface, &present_mode_count, nullptr));
        if (ok()) {
          ASSERT(present_mode_count != 0);
          details.present_modes.resize(present_mode_count);
          VK_FN(vkGetPhysicalDeviceSurfacePresentModesKHR(
              device, m_vk_khr_surface, &present_mode_count,
  ...
  bool swapchain_ok(VkPhysicalDevice device) {
    swapchain_support_details details = query_swapchain_support(device);
  ...
  bool check_device_extensions(VkPhysicalDevice device) {
    ...
    if (ok()) {
      ...
      VK_FN(vkEnumerateDeviceExtensionProperties(device, nullptr,
      ...
      if (ok()) {
        ...
        VK_FN(vkEnumerateDeviceExtensionProperties(
            device, nullptr, &extension_count, avail_ext.data()));
  ...
  void setup_device_and_queues() {
    if (ok_pdev()) {
      ...
      std::set<uint32_t> unique_queue_indices = {
          indices.present_family.value(), indices.graphics_family.value()};

      write_logf("present family queue: %" PRIu32
                 "; graphics family queue: %" PRIu32 "\n",
      ...
      dev_create_info.queueCreateInfoCount =
      ...
      dev_create_info.enabledLayerCount =
          static_cast<uint32_t>(avail_layers.size());
      dev_create_info.ppEnabledLayerNames =
          (!avail_layers.empty()) ? avail_layers.data() : nullptr;
      ...
      VK_FN(vkCreateDevice(m_vk_curr_pdevice, &dev_create_info, nullptr,
                           &m_vk_curr_ldevice));

      if (ok_ldev()) {
        vkGetDeviceQueue(m_vk_curr_ldevice, indices.graphics_family.value(), 0,
                         &m_vk_graphics_queue);

        vkGetDeviceQueue(m_vk_curr_ldevice, indices.present_family.value(), 0,
                         &m_vk_present_queue);
      }
  ...
  bool setup_surface() {
    if (ok()) {
      VK_FN(glfwCreateWindowSurface(m_vk_instance, g_m.device_ctx->window(),
                                    nullptr, &m_vk_khr_surface));
    }
  ...
  VkPresentModeKHR select_present_mode() const {
    ...
    case pms::fifo_relaxed:
      ...
      break;
    default:
      ASSERT(false && "st_config::c_renderer::m_select_present_mode::k_select_"
                      "method desired is not yet implemented");
      break;
    }
  ...
  void setup_swapchain() {
    if (ok_ldev()) {
      if (swapchain_ok(m_vk_curr_pdevice)) {
        swapchain_support_details details =
            query_swapchain_support(m_vk_curr_pdevice);
        ...
        // Here we select the swapchain dimensions (the dimensions of the images
        // that have been rendered into). We want to make them as close as
        // possible to the actual window dimensions; sometimes this will be
        // exact, other times it won't, depending on the system.
        //
        // When the swapchain capabilities are queried, the driver will set the
        // currentExtent width and height to the window dimensions if it asks us
        // to match the dimensions with the window. Otherwise, we have a bit
        // more wiggle room. The driver tells us this by setting
        // currentExtent.width to UINT32_MAX.
        //
        // For now, it's best to keep things simple and be conservative,
        // so we'll operate under the (likely) assumption that UINT32_MAX _is
        // not_ set, and add support for more varied dimension requirements if
        // they pop up in the future.
        ...
        details.capabilities.maxImageCount =
            std::max(details.capabilities.maxImageCount,
        ...
        } else if (m_swapchain_image_count !=
                   st_config::c_renderer::k_desired_swapchain_image_count) {
          m_swapchain_image_count =
              st_config::c_renderer::k_desired_swapchain_image_count;
        }
        ...
        //
        // --
        // On imageUsage: VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT implies that
        // render commands will write directly to the images on the swapchain.
        // If we want to perform offscreen rendering, something like
        // VK_IMAGE_USAGE_TRANSFER_DST_BIT is considered an option.
        //
        // --
        // On imageSharingMode: something to keep in mind for queue family
        // indices is that both the graphics family queue and present family
        // queue can be shared (use the same family index).
        //
        ...
        // queue family at a time and ownership must be explicitly transferred
        // before using it in another queue family; this is what we use if the
        // indices are the same.
        //
        // VK_SHARING_MODE_CONCURRENT implies that images can be used across
        // multiple queue families without explicit ownership transfers. In
        // advance, we specify which queue family indices the transfers will
        // occur between.
        //
        // --
        // On preTransform: represents a transform that is applied to images on
        // the swapchain. Can range from anything to a 90 degreee clockwise
        // rotation or a flip. For now, there is no need, so we simply use the
        // current transform that's provided by the swapchain capabilities.
        //
        // --
        // On compositeAlpha: use VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR to ensure
        // that we don't use the alpha channel to perform a blend operation with
        // other windows in the window system.
        //
        // --
        // On clipped: when other windows in the window system obscure portions
        // of our window, we can use to tell the driver to not draw those
        // portions. While the tutorial this references suggests using VK_TRUE
        // here, we instead use VK_FALSE. Technically speaking there isn't a
        // clear indication that using VK_TRUE will cause issues, but
        // considering that framebuffer reads are crucial to this application,
        // it makes sense to avoid any potential sources of error, as using
        // VK_TRUE will definitely prevent writes to portions of the screen that
        // could be important for post processing (example: post processing that
        // relies on interpolation of already written data that's adjacent to
        // something which is visible, but itself is currently _not_ visible).
        //
        // --
        // On oldSwapChain: at the time of this writing, we keep this set to
        // VK_NULL_HANDLE. This will need to be none-null once we support things
        // like window resizing, or anything that requires re-initialization of
        // the swapchain.
        //
        {
          ...
          VK_FN(vkCreateSwapchainKHR(m_vk_curr_ldevice, &create_info, nullptr,
                                     &m_vk_khr_swapchain));

          if (ok_swapchain()) {
            uint32_t count = 0;
            VK_FN(vkGetSwapchainImagesKHR(m_vk_curr_ldevice, m_vk_khr_swapchain,
                                          &count, nullptr));
  ...
  //
  // --
  // On components: these are designed to specify how different color channels
  // should be used. It's possible for each component to take on the value of,
  // for example, the red channel. It's also possible to set each component to a
  // constant value (e.g., alpha always 1).
  //
  // --
  // On subresourceRange: this describes a) the image's purpose, and b) how it
  // is to be accessed.
  //
  darray<VkImageView> make_image_views(const darray<VkImage> &source_images,
                                       VkFormat format) {
    ...
    for (size_t i = 0; i < ret.size(); ++i) {
      if (ok_swapchain()) {
        ...
        VK_FN(vkCreateImageView(m_vk_curr_ldevice, &create_info, nullptr,
                                &ret[i]));
      }
    }
  ...
  void setup_swapchain_image_views() {
    if (ok_swapchain()) {
      m_vk_swapchain_image_views = make_image_views(
          m_vk_swapchain_images, m_vk_khr_swapchain_format.format);
    }
  }

  std::optional<buffer_data> make_buffer_data(
      VkBufferCreateFlags flags_create, VkBufferUsageFlags flags_usage,
      VkMemoryPropertyFlags flags_memory, VkDeviceSize buffer_size) {
    ...
    if (c_assert(properties.ok())) {
      ...
      std::optional<buffer_reqs> opt_buffreqs = get_buffer_requirements(
          properties, flags_create, flags_usage, flags_memory, buffer_size);
      ...
      if (c_assert(opt_buffreqs.has_value())) {
        ...
        if (c_assert(buffreqs.ok())) {
          ...
          buffer_info.queueFamilyIndexCount =
              properties.queue_family_indices.size();
          buffer_info.pQueueFamilyIndices =
              properties.queue_family_indices.data();
          ...
          if (c_assert(H_OK(ret.handle))) {
            ...
            VK_FN(vkAllocateMemory(m_vk_curr_ldevice, &balloc_info, nullptr,
                                   &ret.memory));

            if (c_assert(H_OK(ret.memory))) {
              VK_FN(vkBindBufferMemory(m_vk_curr_ldevice, ret.handle,
                                       ret.memory, 0));
  ...
  void setup_vertex_buffer() {
    if (ok_graphics_pipeline()) {
      ...
      auto make_and_fill =
        ...
        auto opt_ret =
            make_buffer_data(0, // create flags
                             usage,
        ...
        if (c_assert(opt_ret.has_value())) {
          if (c_assert(opt_ret.value().ok())) {
            ...
            write_device_memory(
                m_vk_curr_ldevice, buffer.memory,
      ...
      STATIC_IF(st_config::c_renderer::m_setup_vertex_buffer::k_use_staging) {
        ...
        auto opt_vertex_buffer = make_buffer_data(
            0, // create flags
            VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                VK_BUFFER_USAGE_VERTEX_BUFFER_BIT,
            VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, k_buffer_size);
        // make sure everything is ok,
        // and then copy from staging to vertex buffer
        good = c_assert(opt_vertex_buffer.has_value()) &&
        ...
        if (good) {
          ...
          run_cmds( // success
              [this, k_buffer_size, &staging](VkCommandBuffer cmd_buf) {
                ...
                vkCmdCopyBuffer(cmd_buf, staging.handle, m_vertex_buffer.handle,
                                1, &region);
              },
              // error
              [this, &good](one_shot_command_error err) { good = false; });

          staging.free_mem(m_vk_curr_ldevice);
        }
      }
      else {
        m_vertex_buffer = make_and_fill(VK_BUFFER_USAGE_TRANSFER_DST_BIT |
                                        VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
  ...
  void run_cmds(one_shot_command_fn_ok_t f, one_shot_command_fn_err_t err_fn) {

    one_shot_command_buffer(make_device_resource_properties(), f, err_fn);
  }
...
public:
  ...
  bool ok_vertex_data() const { return ok() && c_assert(m_ok_vertex_data); }
  ...
  bool is_device_suitable(VkPhysicalDevice device) {
    ...
    queue_family_indices indices =
        query_queue_families(device, m_vk_khr_surface);

    bool extensions_supported = check_device_extensions(device);

    return type_ok && indices.ok() && extensions_supported &&
           swapchain_ok(device);
  }
  ...
  void print_device_info(uint32_t device) {
    if (ok()) {
      if (device < m_vk_physical_devs.size()) {
        ...
        vkGetPhysicalDeviceProperties(m_vk_physical_devs[device], &props);
  ...
  void query_physical_devices() {
    if (ok()) {
      ...
      VK_FN(vkEnumeratePhysicalDevices(m_vk_instance, &device_count, nullptr));

      if (ok()) {
        m_vk_physical_devs.resize(device_count);

        VK_FN(vkEnumeratePhysicalDevices(m_vk_instance, &device_count,
  ...
  darray<const char *> query_layers() {
    ...
    if (ok()) {
      ...
      if (ok()) {
        ...
        VK_FN(
            vkEnumerateInstanceLayerProperties(&layer_count, &avail_layers[0]));
  ...
  void setup_command_pool() {
    if (ok_present()) {
      ...
      VK_FN(vkCreateCommandPool(m_vk_curr_ldevice, &pool_info, nullptr,
                                &m_vk_command_pool));
  ...
  void setup_vertex_data() {
    if (ok_command_pool()) {
      auto add_verts = [this](const std::string &name, mesh_builder &mb,
        ...
        m_vertex_buffer_vertices = m_vertex_buffer_vertices + mb.vertices;
...

      {
        ...
        mb.set_transform(transform().translate(R3v(-2.25, 0, 0))).triangle();

        add_verts("left-triangle", mb, R(1.0));

        // generate right triangle
        mb.set_transform(transform().translate(R3v(2.25, 0, 1)))
        ...
        // generate inner cube
        mb.set_transform(transform().translate(k_mirror_cube_center))
            .cube()
        ...
        // generate sphere
        mb.set_color(R3v(0, 0.5, 0.8))
            .set_transform(transform().translate(R3v(0, 5, 0)))
            .sphere();

        add_verts("sphere", mb, R(1.0));

        // generate outer cube
        mb.set_transform(transform().translate(k_room_cube_center))
            .cube()
  ...
  void setup_descriptor_pool() {
    if (ok_vertex_data()) {
      ...
      uint32_t max_sets = sum<uint32_t, VkDescriptorPoolSize>(
          pool_sizes, [](const VkDescriptorPoolSize &x) -> uint32_t {
      ...
      VK_FN(vkCreateDescriptorPool(m_vk_curr_ldevice, &create_info, nullptr,
  ...
  // path is chosen, then only
  // m_dramebuffer_attachments.data[i].depth_attachment will be used;
  // data[i].color_attachment will remain as VK_NULL_HANDLE.
  //
  enum class pass_type { single, dual_via_input_attachment };
  ...
  void setup_render_pass(pass_type type = pass_type::single) {
    if (ok_descriptor_pool()) {
      ...
      attachments.push_back(
          make_attachment_description(attachment_kind::swapchain));
      ...
      swapchain_attachment_ref.layout =
      ...
      case pass_type::dual_via_input_attachment:
        ...
        attachments.push_back(
            make_attachment_description(attachment_kind::input_color));
        attachments.push_back(
            make_attachment_description(attachment_kind::input_depth));
        ...
        input_attachment_refs[0] = {1,
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
        input_attachment_refs[1] = {2,
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL};
        ...
        subpass_dependencies.push_back(VkSubpassDependency{
            // srcSubpass
            //---------------
        ...
        //
        // Notice that the first subpass dependency given above expects the
        // source, external subpass to have a VK_ACCESS_MEMORY_READ_BIT setting
      ...
      case pass_type::single:
        ...
        attachments.push_back(
            make_attachment_description(attachment_kind::depth));
  ...
  VkAttachmentDescription make_attachment_description(attachment_kind k) {
    ...
    case attachment_kind::swapchain:
      return VkAttachmentDescription{// flags
                                     0,
                                     // format
  ...
  // setup.
  //
  enum class attachment_read_descriptor_type { none, complete };

  void
  setup_attachment_read_descriptors(attachment_read_descriptor_type type =
                                        attachment_read_descriptor_type::none) {
    if (ok_render_pass()) {
      ...
      if (type == attachment_read_descriptor_type::complete) {
        ...
        m_descriptors.attachment_read.resize(
            m_framebuffer_attachments.data.size());

        size_t i{0};
        while (i < m_framebuffer_attachments.data.size() && good) {

          m_descriptors.attachment_read[i] =
              m_descriptor_set_pool.make_descriptor_set(
                  make_device_resource_properties(), attachment_read_params);

          good = m_descriptor_set_pool.ok_descriptor_set(
              m_descriptors.attachment_read.at(i));
          ...
          if (c_assert(good)) {
            ...
            darray<VkDescriptorImageInfo> descriptor_image_infos = {
                ...
                {VK_NULL_HANDLE, color,
                 VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},
                {VK_NULL_HANDLE, depth,
                 VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL},
            };

            VkDescriptorSet descset = m_descriptor_set_pool.descriptor_set(
                m_descriptors.attachment_read.at(i));
  ...
  void setup_uniform_block_data() {
    if (ok_attachment_read_descriptors()) {
      ...
      m_ok_uniform_block_data =
          gen.make<uniform_block::transform>(
              m_transform_uniform_block, uniform_block::k_binding_transform) &&
          gen.make<uniform_block::surface>(m_surface_uniform_block,
                                           uniform_block::k_binding_surface);
    }
  }
    ...
    if (ok_uniform_block_data()) {
      ...
      //
      {
        ...
        m_test_descriptor_set_indices[k_descriptor_set_samplers] =
            m_descriptor_set_pool.make_descriptor_set(
                make_device_resource_properties(), descriptor_set_params);
      }
      ...
      image_gen_params image_params = {// data
      ...
      m_test_image_indices[0] = m_image_pool.make_image(
          make_device_resource_properties(), image_params);
      ...
      m_test_texture_indices[0] = m_texture_pool.make_texture(
          make_device_resource_properties(), texture_params);
      ...
      m_test_image_indices[1] = m_image_pool.make_image(
          make_device_resource_properties(), image_params);

      texture_params.image_index = m_test_image_indices[1];
      texture_params.descriptor_array_element = 1;

      m_test_texture_indices[1] = m_texture_pool.make_texture(
          make_device_resource_properties(), texture_params);
  ...
  bool setup_pipeline(int render_phase_index, int subpass_index,
                      pipeline_layout_gen_params layout_params,
                      pipeline_gen_params params) {
    bool success = false;

    m_pipeline_layout_indices[render_phase_index] =
        m_pipeline_layout_pool.make_pipeline_layout(
            make_device_resource_properties(), layout_params);

    params.subpass_index = subpass_index;
    params.pipeline_layout_index =
        m_pipeline_layout_indices.at(render_phase_index);

    if (m_pipeline_layout_pool.ok_pipeline_layout(
            m_pipeline_layout_indices[0])) {

      m_pipeline_indices[render_phase_index] = m_pipeline_pool.make_pipeline(
          make_device_resource_properties(), params);

      success =
          m_pipeline_pool.ok_pipeline(m_pipeline_indices[render_phase_index]);
    }
  ...
  bool setup_pipeline_texture2d() {
    return setup_pipeline(
        ...
        {// descriptor set layouts
         ...
         {push_constant::basic_pbr_range(), push_constant::model_range()}},
        // pipeline
        {// render pass
         m_vk_render_pass,
         // viewport extent
  ...
  bool setup_pipeline_test_fbo() {
    return setup_pipeline(k_pass_test_fbo,
                          ...
                          1,
                          // pipeline layout
                          {m_descriptor_set_pool.descriptor_set_layouts(
                               m_descriptors.attachment_read),
                           // push constant ranges
                           {}},
                          // pipeline
                          {// render pass
  ...
  //
  enum class pipeline_type { pbr_basic_single, pbr_basic_to_quad };

  void setup_graphics_pipeline(
      pipeline_type type = pipeline_type::pbr_basic_single) {
    if (ok_texture_data()) {
      m_pipeline_pool.set_pipeline_layout_pool(&m_pipeline_layout_pool);

      switch (type) {
      case pipeline_type::pbr_basic_single:
        m_ok_graphics_pipeline = setup_pipeline_texture2d();
        break;
      case pipeline_type::pbr_basic_to_quad:
        m_ok_graphics_pipeline =
            setup_pipeline_texture2d() && setup_pipeline_test_fbo();
        break;
      default:
  ...
  darray<VkFramebuffer>
  make_framebuffer_list(VkRenderPass fb_render_pass,
    ...
    while (i < ret.size() && good) {
      ...
      VK_FN(vkCreateFramebuffer(m_vk_curr_ldevice, &framebuffer_info, nullptr,
                                &ret[i]));
      good = H_OK(ret.at(i));
      i++;
    }
  ...
  //
  enum class framebuffer_setup_method { two_pass, single_pass };

  void setup_framebuffers(framebuffer_setup_method fbmethod =
                              framebuffer_setup_method::single_pass) {
    if (ok_vertex_buffer()) {
      framebuffer_attach_flags_t flags = 0;

      switch (fbmethod) {
      case framebuffer_setup_method::two_pass:
        flags = framebuffer_attach_color_input | framebuffer_attach_depth_input;
        break;
      case framebuffer_setup_method::single_pass:
        flags = framebuffer_attach_depth_output;
        break;
      }

      m_vk_swapchain_framebuffers = make_framebuffer_list(
          m_vk_render_pass, m_vk_swapchain_image_views, flags);

      m_ok_framebuffers = !m_vk_swapchain_framebuffers.empty();
    }
  }

  void commands_begin_pipeline(VkCommandBuffer cmd_buffer, VkPipeline pipeline,
    ...
    vkCmdBindDescriptorSets(cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            pipeline_layout, 0, descriptor_sets.size(),
  ...
  void commands_copy_image(VkCommandBuffer cmd_buffer,
                           image_pool::index_type src,
                           image_pool::index_type dst) const {

    VkImageCopy copy_region = m_image_pool.image_copy(src);

    vkCmdCopyImage(cmd_buffer, m_image_pool.image(src),
                   m_image_pool.layout_final(src), m_image_pool.image(dst),
                   m_image_pool.layout_final(dst), 1, &copy_region);
  }

  void commands_draw_inner_objects(VkCommandBuffer cmd_buffer,
                                   VkPipelineLayout pipeline_layout) const {
    for (auto const &[name, index] : m_model_data.indices) {
      if (name != "outer-cube") {
        commands_draw_model(index, cmd_buffer, pipeline_layout);
      }
    }
  }

  void commands_draw_room(VkCommandBuffer cmd_buffer,
                          VkPipelineLayout pipeline_layout) const {
    commands_draw_model("outer-cube", cmd_buffer, pipeline_layout);
  }
  ...
  void commands_draw_model(uint32_t model, VkCommandBuffer cmd_buffer,
                           VkPipelineLayout pipeline_layout) const {
  ...
  void commands_draw_model(const std::string &name, VkCommandBuffer cmd_buffer,
                           VkPipelineLayout pipeline_layout) const {
    commands_draw_model(m_model_data.indices.at(name), cmd_buffer,
                        pipeline_layout);
  }
  ...
  bool make_command_buffers(darray<VkCommandBuffer> &command_buffers) const {
    bool ret =
        c_assert(!command_buffers.empty()) && c_assert(ok_command_pool());
    ...
    if (ret) {
      ...
      VK_FN(vkAllocateCommandBuffers(m_vk_curr_ldevice, &alloc_info,
  ...
  enum class command_buffer_type { single_pass, two_pass };

  void setup_command_buffers(
      command_buffer_type cmd_type = command_buffer_type::two_pass) {
    ...
    if (ok_framebuffers()) {
      ...
      //
      darray<texture_pool::index_type> tex_indices(
          m_test_texture_indices.begin(), m_test_texture_indices.end());

      // first descriptor set update for textures
      if (c_assert(m_texture_pool.update_descriptor_sets(
              m_vk_curr_ldevice, std::move(tex_indices)))) {
        //
        ...
        run_cmds(
            [this](VkCommandBuffer cmd_buf) {
              ...
              m_ok_scene = m_image_pool.make_layout_transitions(
                  cmd_buf, m_test_image_indices);
            },
        ...
        if (good) {
          ...
          //
          size_t i{0};
          while (i < m_vk_command_buffers.size() && c_assert(good)) {
            ...
            if (good) {
              ...
              commands_begin_pipeline(
                  cmd_buff, pipeline(k_pass_texture2d),
              ...
              commands_draw_main(cmd_buff, pipeline_layout(k_pass_texture2d));
              ...
              if (cmd_type == command_buffer_type::two_pass) {
                ...
                commands_begin_pipeline(cmd_buff, pipeline(k_pass_test_fbo),
                                        pipeline_layout(k_pass_test_fbo),
                                        false, // do not bind vertex buffer
                                        m_descriptor_set_pool.descriptor_sets(
                                            m_descriptors.attachment_read));
  ...
  void setup_sync_objects() {
    if (ok_command_buffers()) {
      ...
      STATIC_IF(
          st_config::c_renderer::m_render::k_allow_more_frames_than_fences) {
        m_vk_images_in_flight.resize(m_vk_swapchain_images.size(),
                                     VK_NULL_HANDLE);
      }
      ...
      for (uint32_t i = 0; i < max_frames_in_flight(); ++i) {
        VK_FN(vkCreateFence(m_vk_curr_ldevice, &fence_info, nullptr,
                            &m_vk_fences_in_flight[i]));

        VK_FN(vkCreateSemaphore(m_vk_curr_ldevice, &semaphore_info, nullptr,
  ...
  void render() {
    if (ok_scene()) {
      ...
      constexpr uint64_t k_timeout_ns =
          16 * 1000000 + 6000000 * 100; // 100 * 16.6 milliseconds

      VK_FN(vkWaitForFences(m_vk_curr_ldevice, 1,
                            &m_vk_fences_in_flight[m_current_frame], VK_TRUE,
                            k_timeout_ns));
      {
        double time = glfwGetTime();
        m_frame_dtimes[m_current_frame] =
            time - m_frame_stimes.at(m_current_frame);
      ...
      VK_FN(vkAcquireNextImageKHR(m_vk_curr_ldevice, m_vk_khr_swapchain,
                                  k_timeout_ns,
      ...
      STATIC_IF(
        ...
        if (m_vk_images_in_flight.at(image_index) != VK_NULL_HANDLE) {
          VK_FN(vkWaitForFences(m_vk_curr_ldevice, 1,
                                &m_vk_images_in_flight[image_index], VK_TRUE,
                                k_timeout_ns));
        }
        ...
        // m_vk_images_in_flight makes sense if and only if we're submitting
        // more frames than we have fences. Otherwise, the image index should
        // directly map to the current frame.
      ...
      if (ok()) {
        ...
        VK_FN(vkResetFences(m_vk_curr_ldevice, 1,
        ...
        VkSemaphore signal_semaphores[] = {
            m_vk_sems_render_finished.at(m_current_frame)};
        submit_info.signalSemaphoreCount = 1;
        submit_info.pSignalSemaphores = signal_semaphores;

        VK_FN(vkQueueSubmit(m_vk_graphics_queue, 1, &submit_info,
                            m_vk_fences_in_flight.at(m_current_frame)));
  ...
  template <class vkHandleType,
            void (*vkDestroyFn)(vkHandleType, const VkAllocationCallbacks *)>
  ...
  template <class vkHandleType,
            void (*vkDestroyFn)(VkInstance, vkHandleType,
  ...
  template <class vkHandleType,
            void (*vkDestroyFn)(VkDevice, vkHandleType,
                                const VkAllocationCallbacks *)>
  ...
  void free_mem() {
    ...
    free_vk_ldevice_handles<VkSemaphore, &vkDestroySemaphore>(
        m_vk_sems_image_available);
    free_vk_ldevice_handles<VkSemaphore, &vkDestroySemaphore>(
        m_vk_sems_render_finished);
    free_vk_ldevice_handles<VkFence, &vkDestroyFence>(m_vk_fences_in_flight);

    free_vk_ldevice_handle<VkCommandPool, &vkDestroyCommandPool>(
        m_vk_command_pool);

    free_vk_ldevice_handles<VkFramebuffer, &vkDestroyFramebuffer>(
    ...
    free_vk_ldevice_handle<VkRenderPass, &vkDestroyRenderPass>(
        m_vk_render_pass);
    ...
    free_vk_ldevice_handle<VkDescriptorPool, &vkDestroyDescriptorPool>(
        m_vk_descriptor_pool);

    free_vk_ldevice_handles<VkImageView, &vkDestroyImageView>(
        m_vk_swapchain_image_views);

    free_vk_ldevice_handle<VkSwapchainKHR, &vkDestroySwapchainKHR>(
        m_vk_khr_swapchain);
    ...
    free_vk_instance_handle<VkSurfaceKHR, &vkDestroySurfaceKHR>(
        m_vk_khr_surface);
